...
function ProcessorStatusBanner({ status }) {
    ...
    if (status.processor_ready) {
      return (
        <div className="flex items-center gap-2 rounded-lg border border-success/30 bg-success-soft px-4 py-3 text-sm text-success">
          ...
          AI Model Active — {displayModelName(status.model_name)} • {status.scale_factor}× super
          resolution{status.device ? ` (${status.device})` : ""}
        </div>
      );
    }
    return (
      <div className="flex items-start gap-2 rounded-lg border border-danger/30 bg-danger-soft px-4 py-3 text-sm text-danger">
        ...
        <span>
          Backend is configured for real AI super-resolution, but the model isn&apos;t available
          {status.processor_error ? `: ${status.processor_error}` : "."} Runs will fail until this
          is fixed.
        </span>
      </div>
    );
  ...
  return (
    <div className="rounded-lg border border-warning/30 bg-warning-soft px-4 py-3 text-sm text-warning">
      ...
      Mock processing active — results are simulated, not generated by a trained AI model.
    </div>
  );
}
...
export default function Process() {
  ...
  // Truth about which processor the backend will actually run — never
  // hardcoded. null while the initial /health check is in flight (or if
  // it fails; the upload/submit flow below surfaces backend-unreachable
  // errors on its own, so this page doesn't need to duplicate that).
  const processorStatus = useProcessorStatus();

  const realModeReady =
    processorStatus?.processor_mode === "super_resolution" && processorStatus?.processor_ready;
  const modelScaleFactor = realModeReady ? processorStatus.scale_factor : null;

  useEffect(() => {
    if (modelScaleFactor && scaleFactor !== modelScaleFactor) {
      setScaleFactor(modelScaleFactor);
    }
    // Only re-run when the model's supported scale becomes known — a user's
    // subsequent manual selection (constrained to it anyway) shouldn't be
  ...
  return (
    <AnimatePresence mode="wait">
                ...
                <div className="flex items-start gap-2 rounded-lg border border-danger/30 bg-danger-soft px-4 py-3 text-sm text-danger">
                  ...
                  <div>
                    <p className="font-medium">
                      {classifyFailure(job)?.title ?? "Processing Failed"}
                    </p>
      ...
      {processorStatus && (
        <ProcessorStatusBanner status={processorStatus} />
      )}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {SCALE_OPTIONS.map((factor) => {
                    // Only the trained model's own scale is enforced here —
                    // the mock pipeline has no such constraint and keeps
                    // supporting every option, as it always has.
                    const disabled = Boolean(modelScaleFactor) && factor !== modelScaleFactor;
                    return (
                      <button
                        key={factor}
                        onClick={() => setScaleFactor(factor)}
                        disabled={disabled}
                        title={disabled ? "Not supported by the active AI model yet" : undefined}
                        className={cn(
                          "relative flex-1 overflow-hidden rounded-lg border px-3 py-2 text-sm font-medium transition-colors",
                          disabled
                            ? "cursor-not-allowed border-border-strong text-text-muted opacity-50"
                            : scaleFactor === factor
                              ? "border-accent/50 text-accent"
                              : "border-border-strong text-text-secondary hover:text-text-primary",
                        )}
                      >
                        {!disabled && scaleFactor === factor && (
                          <motion.span
                        ...
                        <span className="relative">
                          {factor}x{disabled ? " · Coming later" : ""}
                        </span>
                      </button>
                    );
                  })}
